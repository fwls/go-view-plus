<template>
  <div class="go-chart-configurations-data-ajax">
    <n-card class="n-card-shallow">
      <setting-item-box name="请求配置">
        ...
        <setting-item name="方式">
          <n-input size="small" :placeholder="'POST' || '暂无'" :disabled="true"></n-input>
        </setting-item>
      ...
      <setting-item-box name="源名称" :alone="true">
        <n-input size="small" :placeholder="targetData.request.requestDataSourceName || '暂无'" :disabled="true">
          <template #prefix>
    ...
    <!-- 请求配置model -->
    <data-source-request v-model:modelShow="requestShow" :targetData="targetData" @sendHandle="sendHandle" />
  </div>
</template>
...
import { DataSourceRequest } from '../ChartDataRequest/index'
...
const sendHandle = async () => {
  ...
  try {
    ...
    const res = await  executeDataSource(params)
    loading.value = false
    if (res) {
      const { data } = res
