...
import { executeDataSource } from '@/api/path'
...
export const useChartDataFetch = (
  ...
  const requestIntervalFn = () => {
    ...
    // 非请求类型
    if (requestDataType.value !== RequestDataTypeEnum.AJAX && requestDataType.value !== RequestDataTypeEnum.Source)
      return

    try {
      // 处理地址
      // @ts-ignore
      if (requestUrl?.value || requestDataSourceId.value) {
        // requestOriginUrl 允许为空
        // @ts-ignore
        const completePath = requestOriginUrl && requestOriginUrl.value + requestUrl.value
        if (!completePath && !requestDataSourceId.value) return

        clearInterval(fetchInterval)

        const fetchFn = async () => {
          let res = null
          if (requestDataSourceId.value && requestDataSourceId.value != '') {
            const params = {
              id: requestDataSourceId.value,
              sql: requestSQLContent.value.sql
            }
            res = await executeDataSource(params)
          } else {
            res = await customizeHttp(toRaw(targetComponent.request), toRaw(chartEditStore.getRequestGlobalConfig))
          }
          if (res) {
            try {
